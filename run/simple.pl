...
my ($cmd, $path_exe, $path_in, $path_bed, $path_out, $genome, $chipseq_file_old, $chipseq_file_new, $chipseq_file);
...
$gb_limit=           $ARGV[9]; # if a given number of attempts Na to find any background sequence is unsuccessful, then the algorithm is exited (default value 50000)
...
$cmd= "$path_exe/background_genome_mono.exe ${path_in} ${path_out}${chipseq_file}${faext} ${path_out}${chipseq_file}${backext} ${gb_fold} ${gb_at} ${gb_limit} ${genome} ${gb_done} ${path_out}${chipseq_file}${gb_ext1} ${path_out}${chipseq_file}${gb_ext2} ${path_out}${chipseq_file}${gb_ext3} ${path_out}${chipseq_file}${gb_ext4} ${path_out}${chipseq_file}${gb_ext5}";
print "$cmd\n";
system $cmd;
