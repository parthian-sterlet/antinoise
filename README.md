...
9. integer value, length of background sequences. The value 0 means exact correspondence of sequence lengths in the foreground and background sets, any other value greater than zero will set the length of sequences in the background set.
10. output file, additional output table compares the A/T content in the foreground and background sequence sets
11. output file, additional output table compares the dinucleotide frequencies in the foreground and background sequence sets
12. output file, additional output table compares (a) the A/T content for all input sequences that do not achieve the required number of found background sequences per one foreground sequence Rbf and (b) the average A/T content for the foreground set.
13. output file, additional output table compares (a) the dinucleotide frequencies in all input sequences that do not achieve the required number of found background sequences per one foreground sequence Rbf and (b) the average dinucleotide frequencies for the foreground set.
14. output file, current calculation progress, the fraction of completely processed foreground sequences, i.e. for each such foreground sequence exactly Rbf background sequences are found 
...
4. input PLAIN format base name without default extension ".plain" (default value "chr", so that the file names are chr1.plain, chr2.plain, etc.)
5. output PLAIN format base name without default extension ".plain" (default value "chr", so that the file names are chr1.plain, chr2.plain, etc., names are the same as input ones since input and output paths should be distinct)
...
There are three scripts implementing various types of genome masking. The masking of genome may be absetnt ot it can imply either exclusion (blacklisted) or retention (whitelisted). The lengths of sequences in the background set either exactly respect those from the foreground set, or a special parameter defines the same and equal length of all background sequences.
