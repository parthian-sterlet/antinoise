...
Input data of this tool include foreground sequences in FASTA format, and the reference genome sequences in plain format. The [major](https://github.com/parthian-sterlet/antinoise/blob/main/src/background_genome_mono.cpp) propgram runs from a [command_line_simple](https://github.com/parthian-sterlet/antinoise/blob/main/run/command_line_simple). This run requires a version of the reference genome in PLAIN format, this version can be either unmasked or masked. Two perl scripts are proposed to prepare an unmasked version of the reference genome or to apply a particular type of masking to it:
