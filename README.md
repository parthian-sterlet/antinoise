...
+ (c2) either blacklisted or whitelisted masking. The blacklisted masking uses an annotation BED file of 'blacklisted' genomic loci excluding from the entire reference genome suspicious sequences, the extraction procedure is applied to the remaining gemomic loci. The whitelisted masking uses an annotation BED file of 'whitelisted' genomic loci restricting background sequences to only these particular regions, all remaining loci are excluded from analysis. 
The options (c2) provide more rigorous selection of genomic background sequences.
...
All scripts presume input foreground sequences in BED format.
...
## Perl script to extract background sequences the entire reference genome
...
3. path to the rest input data and all output data, these input data includes (a) peaks (foreground sequences) BED file and the whitelist/blacklist BED file, the masked genome all results will in this folder
4. reference genome in FASTA format, e.g. Homo_sapiens.GRCh38.dna.primary_assembly.fa from https://ftp.ensembl.org/pub/release-110/fasta/homo_sapiens/dna/Homo_sapiens.GRCh38.dna.primary_assembly.fa.gz
5. integer value designating blacklisted (0) or whitelisted (1) masking options
6. integer value designating the check of self-overlapping for the BED file applied for masking, e.g. promoters of genes are overlapped but subsequent masking program requires the absence of any overlaps within the same BED track
...
Сommand line examples [command_line_blacklisted](https://github.com/parthian-sterlet/antinoise/blob/main/run/command_line_blacklisted), [command_line_whitelisted](https://github.com/parthian-sterlet/antinoise/blob/main/run/command_line_whitelisted)
Tracks of [simple sequence repeats](https://hgdownload.soe.ucsc.edu/goldenPath/sacCer3/database/simpleRepeat.txt.gz) and [probes from ChIP-chip experiments for transcription regulatory elements](https://hgdownload.soe.ucsc.edu/goldenPath/sacCer3/database/transRegCodeProbe.txt.gz) for the *S. cerevisiae* genome were used as examples for the blacklisted and whitelisted command line run files.
